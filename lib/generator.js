...
async function generateSitemap(options, globalCache, nuxtInstance, depth = 0) {
  ...
  const xmlFilePath = path.join(nuxtInstance.options.dir.static, options.path)
  fs.outputFileSync(xmlFilePath, sitemap.toXML())
  logger.success('Generated', getPathname(nuxtInstance.options.dir.static, xmlFilePath))

  // Generate sitemap.xml.gz
  if (options.gzip) {
    const gzipFilePath = path.join(nuxtInstance.options.dir.static, options.pathGzip)
    fs.outputFileSync(gzipFilePath, sitemap.toGzip())
    logger.success('Generated', getPathname(nuxtInstance.options.dir.static, gzipFilePath))
  }
}
